...
interface FsxFileSystemResource {
  Properties: {
    OntapConfiguration: {
...
describe('Dual FSxN Architecture', () => {
  ...
  test('origin is Multi-AZ', () => {
    const fsList = template.findResources('AWS::FSx::FileSystem') as Record<string, FsxFileSystemResource>;
    const originFs = Object.values(fsList).find((r) => r.Properties.OntapConfiguration.DeploymentType === 'MULTI_AZ_1');
    expect(originFs).toBeDefined();
  });

  test('cache is Single-AZ (no RouteTableIds)', () => {
    const fsList = template.findResources('AWS::FSx::FileSystem') as Record<string, FsxFileSystemResource>;
    const cacheFs = Object.values(fsList).find((r) => r.Properties.OntapConfiguration.DeploymentType === 'SINGLE_AZ_1');
    expect(cacheFs).toBeDefined();
    expect(cacheFs?.Properties.OntapConfiguration.RouteTableIds).toBeUndefined();
  });
