...
パイプラインとアプリケーションの両方を同一アカウント内にでプリロイすることができます。Git リポジトリに対する Push を CodePipeline が検知してそれをトリガーにアプリケーションがパイプラインによって更新されます。以降で示される手順のうち、必須のものを実施した場合にこちらの構成を検証することが可能です。

### 構成パターン B：パイプラインから別アカウントに対してアプリケーションをデプロイする

![BLEA-Deploy-Dev](images/BLEA-DeployECS-03-Dev.png)
パイプラインを持つアカウント（Tools アカウント）とは別のアカウントに対してアプリケーションをデプロイします。以降で示される手順のうち、Optional なものも含めて実施した場合にこちらの構成を検証することができます。
...
パイプライン経由でなく、ローカル環境から直接アプリケーションをデプロイすることも可能です。本構成の手順は Appendix にて示されている手順を実施することにより検証可能です。
...
> ローカルで実行する場合は、 `npm run synth:dev -- --profile xxxxxx`
